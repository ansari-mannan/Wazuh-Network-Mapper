...
async def collect_ifname_map(client, ip: str) -> dict:
    ...
    ifname_rows = await client.walk(ip, fdb.IFNAME_BASE)
    if ifname_rows:
        return fdb.parse_ifnames(ifname_rows)
    ifdescr_rows = await client.walk(ip, fdb.IFDESCR_BASE)
    return fdb.parse_ifnames(ifdescr_rows, base=fdb.IFDESCR_BASE)
...
async def collect_ifindex_by_mac(client, ip: str) -> dict:
    ...
    back to a port name.
    """
    return fdb.parse_ifphys_ifindex(await client.walk(ip, fdb.IFPHYS_BASE))
