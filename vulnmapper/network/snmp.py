...
# LLDP local device: its own advertised system-capabilities bitmap. This is the
# most reliable source of a polled device's OWN role (a neighbor reporting it is
# only available when that neighbor also runs LLDP).
OID_LLDP_LOC_SYS_CAP_ENABLED = "1.0.8802.1.1.2.1.3.6.0"
...
class SnmpClient:
    ...
    async def walk_vlan_context(
        ...
        """Walk ``base_oid`` in the per-VLAN context (the community@vlan trick).
        ...
        ``cyfor123@10``). This builds that contextual community for ``ip``'s
        ...
        if cred is None or cred.version == "v3" or not cred.community:
            return []
        # mpModel=1 = SNMPv2c (required for GETBULK), same as the base credential.
    ...
    def is_v2c(self, ip: str) -> bool:
        ...
        cred = self._resolved.get(ip)
        return bool(cred and cred.version != "v3" and cred.community)
